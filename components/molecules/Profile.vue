		...
		<div class="card-body p-4 min-w-0">
			...
			<div v-else class="space-y-3">
				...
				<div class="form-control">
					...
					<label v-if="profile.profile.githubUsername" class="label py-1 items-baseline">
						<span class="label-text-alt text-success truncate">{{
							$t('profile.connectedAs', { username: profile.profile.githubUsername })
						}}</span>
						<button class="label-text-alt link link-error shrink-0" @click="clearGithub">
							{{ $t('profile.remove') }}
						</button>
					</label>
				</div>
				...
				<div class="flex gap-2">
					<button class="btn btn-primary btn-sm flex-1" @click="save">
						{{ $t('profile.save') }}
					</button>
					<button class="btn btn-ghost btn-sm" @click="profile.setEditing(false)">
						{{ $t('profile.cancel') }}
					</button>
				</div>
			</div>
